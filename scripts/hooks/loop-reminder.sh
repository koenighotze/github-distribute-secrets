...
if [ "$BRANCH" = "main" ]; then
  ...
  FAILING=$(timeout 10 gh pr checks "$PR_NUM" --json state --jq '[.[] | select(.state == "FAILURE" or .state == "PENDING" or .state == "ACTION_REQUIRED")] | length' 2>/dev/null)
  if [ -n "$FAILING" ] && [ "$FAILING" -gt 0 ]; then
    MSG="Open PR #$PR_NUM on '$BRANCH' — CI not yet green. Next: /watch-pr"
  fi
fi
